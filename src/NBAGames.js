...
function NBAGames({ isExpanded, setExpanded }) {
  ...
  const gamesToShow = isExpanded === "NBA" ? orderedGames : orderedGames.slice(0, 3);

  if (isExpanded && isExpanded !== "NBA") return null;
  ...
  return (
    <div className="sports-games nhl-games">
      ...
      {!loading && !error && gamesToShow.length > 0 && (
        <ul className="nba-games-list">
          {gamesToShow.map((game) => (
            <li key={game.gameId} className="nba-game-item">
