...
function CFBGameDetail({ game, onBack }) {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `https://site.api.espn.com/apis/site/v2/sports/football/college-football/summary?event=${game.gameId}`
        );
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.headerGradient}>
        <div style={{
          ...
          <div style={styles.teamSection(isMobile)}>
            ...
            <div style={{ fontSize: isMobile ? "0.95rem" : "1.1rem", fontWeight: "700" }}>
              {awayAbbr}
              {awayRank > 0 && <span style={{ marginLeft: "0.5rem", color: "#10b981" }}>#{awayRank}</span>}
            </div>
          ...
          <div style={styles.teamSection(isMobile)}>
            ...
            <div style={{ fontSize: isMobile ? "0.95rem" : "1.1rem", fontWeight: "700" }}>
              {homeRank > 0 && <span style={{ marginRight: "0.5rem", color: "#10b981" }}>#{homeRank}</span>}
              {homeAbbr}
            </div>
      ...
      {isInProgress && (
        <div style={{
          ...styles.gradientBackground(["#8b5cf6", "#7c3aed"]),
        ...
        <div style={{ display: "grid", gap: "1.5rem" }}>
          <div style={styles.card}>
            ...
            <div style={{ fontSize: "0.95rem", lineHeight: "1.6", opacity: 0.8 }}>
              ...
              {isFinal && (
                <>
                  Final Score: {awayRank > 0 && `#${awayRank} `}{awayAbbr} {awayScore} - {homeScore} {homeRank > 0 && `#${homeRank} `}{homeAbbr}
                </>
              )}
            </div>
          </div>

          {(awayRank > 0 || homeRank > 0) && (
            <div style={styles.card}>
              <div style={styles.cardTitle}>Rankings</div>
              <div style={{ display: "grid", gap: "1rem" }}>
                {awayRank > 0 && (
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    <img src={awayLogo} alt={awayName} style={{ width: "40px", height: "40px", borderRadius: "8px" }} />
                    <div>
                      <div style={{ fontWeight: "700" }}>{awayName}</div>
                      <div style={{ fontSize: "0.9rem", color: "#10b981", fontWeight: "600" }}>Ranked #{awayRank}</div>
                    </div>
                  </div>
                )}
                {homeRank > 0 && (
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    ...
                    <div>
                      <div style={{ fontWeight: "700" }}>{homeName}</div>
                      <div style={{ fontSize: "0.9rem", color: "#10b981", fontWeight: "600" }}>Ranked #{homeRank}</div>
                    </div>
                  </div>
                )}
              </div>
